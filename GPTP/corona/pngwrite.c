   ...
   png_malloc_ptr malloc_fn, png_free_ptr free_fn)
{
   ...
   if (png_ptr->flags & PNG_FLAG_LIBRARY_MISMATCH)
   {
         ...
         (user_png_ver[0] == '0' && user_png_ver[2] < '9'))
     {
        ...
        if (user_png_ver)
        {
          sprintf_s(msg, sizeof(msg), "Application was compiled with png.h from libpng-%.20s",
             user_png_ver);
          png_warning(png_ptr, msg);
        }
        sprintf_s(msg, sizeof(msg), "Application  is  running with png.c from libpng-%.20s",
           png_libpng_ver);
        png_warning(png_ptr, msg);
#endif
