        ...
        compression_state *comp)
{
   ...
   if (compression >= PNG_TEXT_COMPRESSION_LAST)
   {
      ...
      char msg[50];
      sprintf_s(msg, sizeof(msg), "Unknown compression type %d", compression);
   ...
   *new_key = (png_charp)png_malloc(png_ptr, (png_uint_32)(key_len + 2));
   ...
   for (kp = key, dp = *new_key; *kp != '\0'; kp++, dp++)
   {
      if (*kp < 0x20 || (*kp > 0x7E && (png_byte)*kp < 0xA1))
      {
         ...
         sprintf_s(msg, sizeof(msg), "invalid keyword character 0x%02X", *kp);
         png_warning(png_ptr, msg);
#else
...
#else
   sprintf_s(wbuf, sizeof(wbuf), "%12.12e", width);
   sprintf_s(hbuf, sizeof(hbuf), "%12.12e", height);
#endif
