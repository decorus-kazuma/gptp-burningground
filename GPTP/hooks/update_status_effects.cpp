...
void doIrradiateDamage(CUnit *irradiatedUnit) {
  ...
  auto irradiateProc = [&irradiatedUnit] (CUnit *unit) {
    ...
    {
      const u32 damage = weapons_dat::DamageAmount[WeaponId::Irradiate] * 256 / weapons_dat::Cooldown[WeaponId::Irradiate];
...
void updateStatusEffectsHook(CUnit *unit) {
  ...
  if (unit->acidSporeCount) {
    ...
    u32 acidOverlayId = std::min(3, unit->acidSporeCount >> 1)
