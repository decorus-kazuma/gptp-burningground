...
//화면 흔들림 효과 함수
void shockScreen(u8 *shock, u8 *power, s16 saveXY[], s16 restoreXY[]){
	u32 moveX, moveY;
	s8 randomX, randomY;

	//흔들림 마지막 단계에선 원위치로 돌림
	if(*shock == 1){
		updateScreenPosition((u32)std::max(std::min((int)*screenX - restoreXY[0], (int)*maxX), 0), (u32)std::max(std::min((int)*screenY - restoreXY[1], (int)*maxY), 0));
		saveXY[0] = saveXY[1] = restoreXY[0] = restoreXY[1] = 0;
		*power = 0;
	}else{
...
s16 saveXY[2] = {0, 0};//흔들리는 동안 이동한 값을 저장하여 다음 흔들림의 방향을 결정할 때 사용되는 변수
s16 restoreXY[2] = {0, 0};//흔들림 효과가 끝난 후 원래 자리로 복구시키기 위한 변수
...
void setShock(unsigned char strength, unsigned short x, unsigned short y){
	...
	if(shockCheck <= strength)
		shock = power = (u8)std::max(std::min((int)(strength-shockCheck)*3, 127), 2);
}
...
u32 myunitEx = 0;
//u16 unitList[UNIT_TYPE_COUNT] = {0,}; //쓸데나 주석처리 해제하고 써라
...
bool nextFrame() {
	...
	if(shock){
		...
		shockScreen(&shock, &power, saveXY, restoreXY);
	}
	...
	for (int i = 0, j = 0; j < *clientSelectionCount && i < SELECT_UNIT_COUNT; ++i) {
		...
		if(obMode == true){
			...
			if(drawMove == true){
				//QOrder
				if(scbw::isInReplay() || scbw::isAlliedTo(selUnit->playerId,*LOCAL_HUMAN_ID) ) {

					if(selUnit->status & UnitStatus::InAir)			//Terran Flying Buildings especially
						DrawTargetedOrder(selUnit);
					else
					if(units_dat::GroupFlags[selUnit->id].isFactory)	//Rally point case
						DrawRallyPoint(selUnit);
					else
					if(selUnit->hasWeapon() || !units_dat::GroupFlags[selUnit->id].isBuilding)	//include armed building, exclude others buildings and include all other units
						DrawTargetedOrder(selUnit);
			...
			switch(selUnit->id){
				...
				if(selUnit->defensiveMatrixHp > 0 && selUnit->defensiveMatrixTimer > 0){
					sprintf_s(buffer, "\x04 Defensive Matrix HP: %d \x01", selUnit->defensiveMatrixHp >> 8);
					graphics::drawText(selUnit->getX()-76, selUnit->getY()+42, buffer, graphics::FONT_MEDIUM, graphics::ON_MAP);
					sprintf_s(buffer, "\x1F   Available Time: %d sec \x01", selUnit->defensiveMatrixTimer / 3);
					graphics::drawText(selUnit->getX()-76, selUnit->getY()+52, buffer, graphics::FONT_MEDIUM, graphics::ON_MAP);
				}//배틀 디펜시브 정보 출력
				break;
			...
			case UnitId::sarah_kerrigan:
				if(*clientSelectionCount == 1){
					...
					u8 maxWeaponRange = selUnit->getMaxWeaponRange(units_dat::GroundWeapon[selUnit->id]);
					...
					spTarget.forEach([selUnit, maxWeaponRange, X_Dir, Y_Dir, X_LimitR, Y_LimitR, mtargets] (CUnit *spTarget) {
						...
						if (scbw::getDistanceFast(selUnit->getX(), selUnit->getY(), spTarget->getX(), spTarget->getY()) <= maxWeaponRange
					...
					if((selUnit->mainOrderId == OrderId::AttackUnit || selUnit->mainOrderId == OrderId::HoldPosition2) 
						...
						for (int i = 0; i < mtargets; ++i) {
							...
							if (scbw::getDistanceFast(selUnit->getX(), selUnit->getY(), tunit->getX(), tunit->getY()) <= maxWeaponRange
								&& scbw::getDistanceFast(X_Dir, Y_Dir, X_Target, Y_Target) <= scbw::getDistanceFast(X_Dir, Y_Dir, X_LimitR, Y_LimitR)
								&& tunit->hitPoints > 0
		...
		if (!(*IS_IN_REPLAY)) {   //리플레이 시청 모드에서는 안 씀
			...
			if (unit->playerId == *LOCAL_HUMAN_ID) {						//현재 플레이어 소유의 유닛인지 확인
				if (units_dat::BaseProperty[unit->id] & UnitProperty::Worker){  //일꾼인지 확인
					if(unit->mainOrderId == OrderId::PlayerGuard)           //놀고 있는 상태인지 확인
						++idleWorkerCount;
					//실험건물 카운트
				}
				...
				if(vikingButtonChange){
					...
					case UnitId::aldaris://바이킹(공중모드)
						if(unit->currentButtonSet != 228//락다운 상태같이 버튼이 없는 상태에선 적용안함
							&& igo_D && igo_O){//방어모드 상태와 공격모드 상태인 이고르가 같이 포함되어 있는가
	...
	*/
	switch(unit->id){
		...
		case UnitId::mojo:
			if(unit->_padding_0x132 > 0)
				BomberAI(unit);
		break;
		case UnitId::cargo_ship:
			if(unit->_padding_0x132 == 1 && unit->sprite->mainGraphic->animationEx == IscriptAnimation::WalkingToIdle){
				scbw::createUnitAtPos(UnitId::gerard_dugalle, unit->_unused_0x106, unit->getX(), unit->getY());
				scbw::createUnitAtPos(UnitId::gerard_dugalle, unit->_unused_0x106, unit->getX(), unit->getY());
				unit->_padding_0x132 = 2;
				const MapSize map = *mapTileSize;
				u32 rtnX = 0;
				u32 rtnY = 0;
				u8 minVal = 50;
				u32 xLim = map.width<<5;
				u32 yLim = map.height<<5;
				Target target = unit->orderTarget;
				switch(scbw::randBetween(1,4)){
					case 1:
						rtnX = minVal;
						rtnY = scbw::randBetween(minVal, yLim-minVal);
						break;
					case 2:
						rtnX = scbw::randBetween(minVal, xLim-minVal);
						rtnY = minVal;
						break;
					case 3:
						rtnX = xLim-minVal;
				...
				unit->orderTo(OrderId::Move, rtnX, rtnY);
				unit->secondaryOrderPos.x = rtnX;
				unit->secondaryOrderPos.y = rtnY;
			}
			else if(unit->_padding_0x132 == 2 && scbw::getDistanceFast(unit->orderTarget.pt.x, unit->orderTarget.pt.y, unit->getX(), unit->getY()) < 32){
				unit->userActionFlags |= 4;
				unit->remove();
			}
		break;
		...
		case UnitId::battlecruiser:
			if (unit->mainOrderId == OrderId::CarrierStop || unit->mainOrderId == OrderId::DefensiveMatrix){
				if(unit->energy >= techdata_dat::EnergyCost[TechId::DefensiveMatrix]<<8)
		...
		case UnitId::jim_raynor_vulture:
			//공격 대상이 건물인지 확인
			if (unit->mainOrderId == OrderId::AttackUnit && unit->orderTarget.unit && (unit->orderTarget.unit->status & UnitStatus::GroundedBuilding) 
				&& scbw::getDistanceFast(unit->getX(), unit->getY(), unit->orderTarget.unit->getX(), unit->orderTarget.unit->getY())
				<= unit->getMaxWeaponRange(units_dat::GroundWeapon[unit->id])+96) {
				//공격 중인지 확인
				const u8 currentAnim = unit->sprite->mainGraphic->animationEx;
				if (currentAnim == IscriptAnimation::GndAttkInit
				 || currentAnim == IscriptAnimation::GndAttkRpt) {
					createBullet(128, unit, unit->position.x, unit->position.y, unit->playerId, unit->currentDirection1); //무기ID == 128
					unit->playIscriptAnim(IscriptAnimation::CastSpell);
					unit->groundWeaponCooldown = weapons_dat::Cooldown[128];
				}
			}
		break;
		...
		case UnitId::jim_raynor_marine:
			...
			if (unit->mainOrderId == OrderId::AttackUnit && unit->orderTarget.unit && (unit->orderTarget.unit->status & UnitStatus::GroundedBuilding) 
				...
				if (currentAnim == IscriptAnimation::GndAttkInit
				 || currentAnim == IscriptAnimation::GndAttkRpt) {
					createBullet(129, unit, unit->position.x, unit->position.y, unit->playerId, unit->currentDirection1); //무기ID == 129
					unit->playIscriptAnim(IscriptAnimation::Unused2);
					unit->groundWeaponCooldown = weapons_dat::Cooldown[129];
				}
			}
			//리퍼 점프코드. 최적화 완료됨
					...
					if (unit->mainOrderId == OrderId::AttackUnit 
						...
						&& !(unit->status & UnitStatus::InAir)) {
							if(unit->beacon._unknown_00 == (u32)unit->orderTarget.unit){
								if(unit->energy >= 64 || isCheatEnabled(TheGathering)){
									unit->playIscriptAnim(IscriptAnimation::Unused1);
								}
							}
							else{
								unit->beacon._unknown_00 = NULL;
								if(scbw::getDistanceFast(unit->orderTarget.unit->getX(), unit->orderTarget.unit->getY(), unit->getX(), unit->getY()) <= 384 
									&& (unit->energy == 2560 || isCheatEnabled(TheGathering))){
		...
		case UnitId::dragoon:
			if(unit->mainOrderId == OrderId::Consume){
				if(unit->orderTarget.pt.x != unit->getX() && unit->orderTarget.pt.y != unit->getY()){ //시전 지점이 자신의 위치면 명령 무시
					if(unit->energy >= techdata_dat::EnergyCost[TechId::Consume]<<8 || isCheatEnabled(TheGathering)){//마나가 10 이상이거나 마나무한 치트가 있으면
						...
						if(!(actTile.exploredFlags & (1 << unit->playerId)) //탐색되지 않은 지형에다가 점멸할 수 없음.
							...
							if(scbw::getDistanceFast(unit->getX(), unit->getY(), unit->orderTarget.pt.x, unit->orderTarget.pt.y) <= 176) {//대상 거리가 160 이하면
		...
		case UnitId::high_templar:
			...
			case OrderId::PsiStorm:
				if(unit->orderTarget.unit && unit->getDistanceToTarget(unit->orderTarget.unit) <= unit->getMaxWeaponRange(WeaponId::PsiStorm)){
					if(unit->energy >= (techdata_dat::EnergyCost[TechId::PsionicStorm]<<8) || isCheatEnabled(TheGathering)){
						unit->connectedUnit = unit->orderTarget.unit;
			...
			if(unit->building.silo.isReady){
				if(!isCheatEnabled(TheGathering))
					unit->energy = (u16)std::max((s32)unit->energy-32, 0);
				if(!unit->energy){
					unit->building.silo.isReady = false;
					CImage *CImg = unit->sprite->getOverlay(449);
					if(CImg)
			...
			if(unit->connectedUnit){
				if(unit->_unused_0x106 && !unit->unusedTimer){
					...
					unit->orderTarget.unit = unit->connectedUnit;
					createBullet(121, unit, unit->getX(), unit->getY(), unit->playerId, unit->currentDirection1);
		...
		case UnitId::dark_archon:
			if (unit->mainOrderId == OrderId::ReaverStop){
				if(unit->energy >= techdata_dat::EnergyCost[TechId::DarkArchonMeld]<<8 || isCheatEnabled(TheGathering)){
					if(!isCheatEnabled(TheGathering))
		...
		case UnitId::dark_templar_hero:
			...
			if(unit->orderSignal & 1){
				...
				unit->sprite->setDirectionAll(unit->currentDirection1);
				unit->psi_link.prev = unit->psi_link.next = nullptr;
				unit->updateSpeed();
			}
		break;
		...
		case UnitId::corsair:
			if (unit->mainOrderId == OrderId::CarrierStop || unit->mainOrderId == OrderId::CastMindControl){
				if(unit->energy >= techdata_dat::EnergyCost[TechId::MindControl]<<8 || isCheatEnabled(TheGathering)){
					if(!isCheatEnabled(TheGathering))
						unit->energy -= techdata_dat::EnergyCost[TechId::MindControl]<<8;
		...
		case UnitId::infested_duran:
			...
			if(unit->orderSignal & 0x10){
				...
				if(isCollide){
					...
					const s32 saveShield = unit->shields;
							...
							if(!actTileA.isUnwalkable && !actTileA.cliffEdge && !actTileB.isUnwalkable && !actTileB.cliffEdge){
								...
								if(dropUnitA || dropUnitB){
									if(dropUnitA){
										...
										raceSupply[CUnit::getRace(UnitId::gerard_dugalle)].used[dropUnitA->playerId] += units_dat::SupplyRequired[UnitId::gerard_dugalle];
									...
									if(dropUnitB){
										...
										raceSupply[CUnit::getRace(UnitId::gerard_dugalle)].used[dropUnitB->playerId] += units_dat::SupplyRequired[UnitId::gerard_dugalle];
				...
				//폭격기 호출
					...
					if(unit->_unknown_0x066 > 0 || isCheatEnabled(TheGathering)){
						...
						if(bomber){
							...
							marker->sprite->mainGraphic->coloringData = colorShiftData[ColorRemapping::OFire].data;
		...
		case UnitId::UnusedTerran1:
			if(unit->status & UnitStatus::Completed && !(unit->status & UnitStatus::DoodadStatesThing)){
				...
				if(unit->mainOrderId == OrderId::Ensnare) {
					if(unit->_unknown_0x066 || isCheatEnabled(TheGathering)){
						Target target = unit->orderTarget;
						for(int i = 0; i < PLAYER_COUNT; i++){
							if(i != unit->playerId){
								minimapPing(target.pt.x, target.pt.y, 0x11);
						...
						unit->orderTo(OrderId::Plague, unit->orderTarget.pt.x, unit->orderTarget.pt.y);
						if(!isCheatEnabled(TheGathering))
							--unit->_unknown_0x066;
					}
					else{
				...
				if(findComsat == true){//컴셋 찾을 때
					for(CUnit* searchAddon=*firstVisibleUnit;searchAddon;searchAddon=searchAddon->link.next){
						if(searchAddon->playerId == 11
							...
							&& (searchAddon->id == UnitId::comsat_station || searchAddon->id == 181)){
								searchAddon->id = changeId;
					...
					for(CUnit* searchAddon=*firstVisibleUnit;searchAddon;searchAddon=searchAddon->link.next){
						if(searchAddon->playerId == 11
							...
							|| searchAddon->id == UnitId::control_tower || searchAddon->id == UnitId::control_tower 
							|| searchAddon->id == UnitId::covert_ops || searchAddon->id == UnitId::Special_IndependentStarport 
							|| searchAddon->id == UnitId::UnusedIndependentJumpGate)){
								searchAddon->id = changeId;
								searchAddon->currentButtonSet = changeId;
								break;
						}
		...
		case 158:{//역장
			if(unit->shields < 256 && units_dat::MaxShieldPoints[unit->id])
				unit->remove();
    ...
    for (CBullet* bullet = *firstBullet; bullet; bullet = bullet->next) {
		...
		if(bullet->sourceUnit != nullptr){
			switch(bullet->sourceUnit->id) {
			case UnitId::alexei_stukov://센츄리온이 타겟없이 부르드링 시전했을때 생성
				if(bullet->weaponType == WeaponId::SpawnBroodlings){
					if(bullet->moveTarget.unit == NULL && bullet->attackTarget.unit == NULL) {
						if(bullet->targetPosition.x == bullet->position.x && bullet->targetPosition.y == bullet->position.y) {
							scbw::createUnitAtPos(UnitId::broodling,bullet->sourceUnit->playerId,bullet->targetPosition.x,bullet->targetPosition.y)->removeTimer = 500;
						}
					}
				}
				break;
			case UnitId::gerard_dugalle://포물선 탄 플러그인
	...
	if (needRefreshConsole){
		scbw::refreshConsole();
		needRefreshConsole = false;
	}

	if (debugMode){
		if(debugMode == 2){
			if(*clientSelectionCount == 1 && clientSelectionGroup->unit[0]){
				sprintf_s(buffer, sizeof(buffer), "MainOrderId: %3d", clientSelectionGroup->unit[0]->mainOrderId);
				graphics::drawText(527, 40, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "secondaryOrderId: %3d", clientSelectionGroup->unit[0]->secondaryOrderId);
				graphics::drawText(495, 52, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "Status: 0x%08x", clientSelectionGroup->unit[0]->status);
				graphics::drawText(527, 64, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "orderTargetUnit: %3d", clientSelectionGroup->unit[0]->orderTarget.unit);
				graphics::drawText(500, 76, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "connectedUnit: %3d", clientSelectionGroup->unit[0]->connectedUnit);
				graphics::drawText(500, 88, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "currentBuildUnit: %3d", clientSelectionGroup->unit[0]->currentBuildUnit);
				graphics::drawText(500, 100, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "addon: %3d", clientSelectionGroup->unit[0]->building.addon);
				graphics::drawText(500, 112, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "currentButtonSet: %3d", clientSelectionGroup->unit[0]->currentButtonSet);
				graphics::drawText(500, 124, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "orderSignal: %x", clientSelectionGroup->unit[0]->orderSignal);
				graphics::drawText(500, 136, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "visibilityStatus: 0x%08x", clientSelectionGroup->unit[0]->visibilityStatus);
				graphics::drawText(475, 148, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "MovementFlags: 0x%08x", units_dat::MovementFlags[clientSelectionGroup->unit[0]->id]);
				graphics::drawText(475, 160, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "currentDirection1: %3d", clientSelectionGroup->unit[0]->currentDirection1);
				graphics::drawText(500, 172, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "currentDirection2: %3d", clientSelectionGroup->unit[0]->currentDirection2);
				graphics::drawText(500, 184, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "velocityDirection1: %3d", clientSelectionGroup->unit[0]->velocityDirection1);
				graphics::drawText(500, 196, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "velocityDirection2: %3d", clientSelectionGroup->unit[0]->velocityDirection2);
				graphics::drawText(500, 208, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "Animation: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->animation);
				graphics::drawText(4, 0, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "iscriptHeaderOffset: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->iscriptHeaderOffset);
				graphics::drawText(4, 12, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "iscriptOffset: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->iscriptOffset);
				graphics::drawText(4, 24, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "frameSet: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->frameSet);
				graphics::drawText(4, 36, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "frameIndex: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->frameIndex);
				graphics::drawText(4, 48, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "horizontalOffset: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->horizontalOffset);
				graphics::drawText(4, 60, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "verticalOffset: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->verticalOffset);
				graphics::drawText(4, 72, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "mainGraphicId: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->id);
				graphics::drawText(4, 108, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "paletteType: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->paletteType);
				graphics::drawText(4, 120, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "direction: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->direction);
				graphics::drawText(4, 132, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "mainGraphic flags: 0x%04x", clientSelectionGroup->unit[0]->sprite->mainGraphic->flags);
				graphics::drawText(4, 144, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "flags: 0x%04x", clientSelectionGroup->unit[0]->sprite->flags);
				graphics::drawText(4, 156, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "visibilityFlags: 0x%04x", clientSelectionGroup->unit[0]->sprite->visibilityFlags);
				graphics::drawText(4, 168, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "elevationLevel: %2d", clientSelectionGroup->unit[0]->sprite->elevationLevel);
				graphics::drawText(4, 180, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "index: %u", clientSelectionGroup->unit[0]->sprite->index);
				graphics::drawText(4, 192, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "playerId: %3d", clientSelectionGroup->unit[0]->sprite->playerId);
				graphics::drawText(4, 204, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "spriteId: %3d", clientSelectionGroup->unit[0]->sprite->spriteId);
				graphics::drawText(4, 216, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "selectionIndex: %3d", clientSelectionGroup->unit[0]->sprite->selectionIndex);
				graphics::drawText(4, 228, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "selectionTimer: %3d", clientSelectionGroup->unit[0]->sprite->selectionTimer);
				graphics::drawText(4, 240, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "position.x: %3d", clientSelectionGroup->unit[0]->sprite->position.x);
				graphics::drawText(4, 252, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "position.y: %3d", clientSelectionGroup->unit[0]->sprite->position.y);
			...
			if(*clientSelectionCount == 1 && clientSelectionGroup->unit[0]){
				...
				sprintf_s(buffer, sizeof(buffer), "iscriptHeaderOffsetEx: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->iscriptHeaderOffsetEx);
				graphics::drawText(476, 64, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "iscriptOffsetEx: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->iscriptOffsetEx);
				graphics::drawText(490, 76, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "unknown2Ex: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->unknown2Ex);
				graphics::drawText(500, 88, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "animationEx: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->animationEx);
				graphics::drawText(500, 100, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
				sprintf_s(buffer, sizeof(buffer), "waitEx: %2d", clientSelectionGroup->unit[0]->sprite->mainGraphic->waitEx);
				graphics::drawText(500, 112, buffer, graphics::FONT_MEDIUM, graphics::ON_SCREEN);
			}
		}
