...
BOOL Plugin::checkStarCraftExeVersion(LPCSTR exePath) const {
  ...
  if (fileVersionInfoBufferSize > 0) {
      ...
      && VerQueryValue(fileVersionInfoBuffer, "\\", (LPVOID*)&fileInfo, &fileInfoSize))
    {
      ...
      sprintf_s(errorMessage, sizeof(errorMessage), "Cannot retrieve version information from:\n  %s", exePath);
    }
    delete [] fileVersionInfoBuffer;
  }
  else {
    sprintf_s(errorMessage, sizeof(errorMessage), "Cannot retrieve version information size from:\n  %s", exePath);
  }
